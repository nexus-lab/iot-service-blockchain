...
func (s *ServiceRegistrySmartContract) Deregister(ctx TransactionContextInterface, name string) error {
	var err error
	...
	if service, err = ctx.GetServiceRegistry().Get(organizationId, deviceId, name); err != nil {
		return err
	}
